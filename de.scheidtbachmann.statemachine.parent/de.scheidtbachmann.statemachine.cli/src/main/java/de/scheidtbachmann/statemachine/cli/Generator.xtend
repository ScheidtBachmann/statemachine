...
@Command(name='scc', header="Scheidt & Bachmann StateMachine Compiler", version="0.2.0", mixinStandardHelpOptions=true)
class Generator implements Runnable {
     ...
     */
    @Command(name=CMD_VALIDATE, header='Check the given input file for syntactic and semantic errors and problems.', 
        parameterListHeading="%nParameters:%n", optionListHeading="%nOptions:%n", sortOptions=false)
    def void validate(
     ...
     */
    @Command(name=CMD_GENERATE, separator=" ", header='Generate executable code corresponding to the given input file.',
        parameterListHeading="%nParameters:%n", optionListHeading="%nOptions:%n", sortOptions=false)
    def void generate(
        @Option(names='--stdin', description="Forces the generator to read input from stdIn.")
        boolean stdIn,
        @Option(names=#['-o', '--output'], paramLabel='<path>', defaultValue='gen', 
        ...
        boolean stdOut,
        @Option(names=#['-s', '--strategy'], paramLabel='<strategy>', completionCandidates=StrategyCandidates, 
            defaultValue='de.cau.cs.kieler.sccharts.statebased', 
            description="The strategy to apply.%nOptions: ${COMPLETION-CANDIDATES}.%nDefault:%n  ${DEFAULT-VALUE}.")
        String strategy,
        @Option(names='--select', 
            description="The parts of the model that should be taken from the input file", paramLabel='<model>')
     ...
     */
    def doCompile(String strategyId, String selectedCharts, ByteArrayOutputStream altSysout,
        ByteArrayOutputStream altSyserr) {
     ...
     */
    @Command(name=CMD_DRAW, separator=" ", header='Draw the state chart specified in given input file.',
        parameterListHeading="%nParameters:%n", optionListHeading="%nOptions:%n", sortOptions=false)
    def void draw(
        @Option(names='--stdin', description="Forces the artist to read input from stdIn.")
        boolean stdIn,
        @Option(names=#['-f', '--format'], paramLabel='<format>', defaultValue='png', 
            completionCandidates=FormatCandidates, 
            description="The format of the images.%nCandidates: ${COMPLETION-CANDIDATES}.%nDefault: ${DEFAULT-VALUE}.")
        String format,
        @Option(names=#['-o', '--output'], paramLabel='<path>', defaultValue='diagrams', 
